...
test.describe('SauceDemo authentication', { tag: ['@ui', '@auth'] }, () => {
  test('standard user can log in and land on inventory', { tag: ['@smoke'] }, async ({ loginPage, inventoryPage }) => {
  ...
  test('locked out user sees a clear login error', { tag: ['@negative'] }, async ({ loginPage }) => {
    await loginPage.goto();
  ...
  test('unauthenticated user cannot deep-link into inventory', { tag: ['@negative', '@session'] }, async ({ loginPage }) => {
